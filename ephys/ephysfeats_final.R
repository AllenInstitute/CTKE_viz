...
patchtypenames <- unique(patch_metadata$RNA.type)
...
var.labs <- c(AP.amplitude..mV. = "AP Amplitude (mv)", 
              AP.width..ms. = "AP Width (ms)", 
              Input.resistance..MOhm. = "Input Resistance (MOhms)",
              Max.number.of.APs = "Max # of APs", 
              Membrane.time.constant..ms. = "Membrane Time Constant (ms)")
...
plot_ephys_feats <- function(data,
  ...
  resist.plot <- data %>%
    ...
    ggplot(aes(x = X_plot, y = Input.resistance..MOhm.,
               fill = X_plot)) +
    
    
    geom_violin(alpha=0.9,scale = "width", width = 1) + 
    geom_jitter(fill = "black", width = 0.04, size=0.2) +
    ...
    theme(text = element_text(size=8),
          ...
          axis.title.y.left = element_text(margin = margin(0, 12, 0, 0)),
  ...
  amp.plot <- data %>%
    ...
    ggplot(aes(x = X_plot, y = AP.amplitude..mV.,
               fill = X_plot)) +
  ...
  time.plot <- data %>%
    ...
    ggplot(aes(x = X_plot, y = Membrane.time.constant..ms.,
               fill = X_plot)) +
    ...
    labs(title = "Membrane Time Constant",
         y = "ms",
         x = "") +
  ...
  num.plot <- data %>%
    ...
    ggplot(aes(x = X_plot, y = Max.number.of.APs,
               fill = X_plot)) +
  ...
  width.plot <- data %>%
    ...
    ggplot(aes(x = X_plot, y = AP.width..ms.,
               fill = X_plot)) +
  ...
  up.plot <- data %>%
    ...
    ggplot(aes(x = X_plot, y = Upstroke.to.downstroke.ratio,
               fill = X_plot)) +
    ...
    labs(title = "Upstroke:Downstroke Ratio",
         y = "Ratio",
         x = "") +
  ...
  g <- grid.arrange(arrangeGrob(resist.plot,amp.plot,
                                time.plot, num.plot,
                                width.plot,up.plot),
                    widths=c(2))
  g
  ...
  ggsave(g, filename = paste0(outname,nodename,"_characteristics.svg"))
  write.table(data, paste0(outname, nodename, "_characteristics_data.csv"), sep=",", 
              row.names = F)
...
dat_dir <- "//allen/programs/celltypes/workgroups/humancelltypes/RayS/CTCard_Content/ephys/cardcontent"
...
for (i in 1:length(patchglutsub)) {
  ...
  if (subsize == 1 & length(checktypes) == 0) {
    ...
    plot_ephys_feats(data = merged_dat, 
                     ...
                     X_plot = subclass_label, 
                     level_id = subclass_id,
                     outname = "electrophysiology__TYPE_",
...
missingsubs <- setdiff(as.character(gabasubnames), unique(as.character(merged_dat$subclass_label)))
missingnodes <- setdiff(as.character(gabatypenames), unique(as.character(merged_dat$cluster_label)))

missinggaba <- c(missingsubs, missingnodes)
write.table(c(missingglut,missinggaba),"Patch-Seq Missing Taxons.txt")
