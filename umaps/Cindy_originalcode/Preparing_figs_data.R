...
source("//allen/programs/celltypes/workgroups/mct-t200/Cindy_analysis/my_Tools/molgen-shiny/distillery/annocomp_functions.R")
...
for(n in clades){
  ...
  sub.anno = anno[anno$neighborhood_label==n,]
  
  prefix <- sub("-", "_", n)
  genes_use = marker.genes[[n]]
  
  genes_df <- mouse_data[rownames(mouse_data) %in% genes_use,colnames(genes_df) %in% sub.anno$sample_id ]
  genes_df <- as.data.frame(as.matrix(genes_df))
  cpm_dat_tbl_orig <- as_tibble(genes_df, rownames = "gene")
  
    cpm_dat_tbl_long <- cpm_dat_tbl_orig %>% 
    gather(key = "sample_id",
           value = "cpm_expr",
           2:ncol(.))
  
  
    to_add <- sub.anno %>% select(sample_id, cluster_label, cluster_color, cluster_id, neighborhood_label,neighborhood_color)
    
   to_plot <- cpm_dat_tbl_long %>%
      left_join(to_add) %>%
      arrange(cluster_label) %>%
      mutate(gene = as_factor(gene) %>% 
               fct_relevel(genes_use)) 
    
    
    to_plot_stats <- to_plot %>% 
        arrange(cluster_id)  %>%
  
  #find proportion of subclass that express gene for each species
  group_by(cluster_label, cluster_color, gene) %>%
  summarise(cluster_prop = sum(cpm_expr > 1)/n_distinct(sample_id),
            cluster_mean_expr = mean(cpm_expr)) %>%
  ungroup() %>%
  
  #scale expression for each species for each gene from 0 to 1
  group_by(gene) %>%
  mutate(cluster_mean_expr_scaled = 
           ( (cluster_mean_expr - min(cluster_mean_expr)) / (max(cluster_mean_expr) - min(cluster_mean_expr))
           )  )
    ...
    plot <- to_plot_stats %>%
      ...
      ggplot(aes(x = cluster_label, 
                 y=1,
                 color = cluster_mean_expr_scaled,
                 size = cluster_prop)) +
      ...
      facet_grid(factor(gene, levels = genes_use) ~ 1, scales = "free_y") +
    ...
    png(filename = paste0(out.dir, "/Plots/dotplot_",prefix,"_markers.png"),
        height = 700,
        width = 600)
    print(plot)
    dev.off()
